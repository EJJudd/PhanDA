...
figname = 'SupFig_PaleoVeizer_Zonal.png';
savefig = false;
...
for ii = 1:numel(PaleoClump.PublishedSST)
    lidx = find(abs(Lat-PaleoClump.PaleoLat(ii)) == min(abs(Lat-PaleoClump.PaleoLat(ii))));
    sidx = find(contains(GTS.Stage, PaleoClump.Stage(ii)));
    sltg = LTGsst{sidx};
    vltg = LTGsst_Veizer{sidx};
    PhanDAsst(ii,:) = prctile(sltg(lidx,:),P,'all');
    Veizersst(ii,:) = prctile(vltg(lidx,:),P,'all');
end
...
ylim(xlim)
plot(xlim,ylim,'k--')
set(gca,'FontSize',11,'YTick',[0:10:50])
